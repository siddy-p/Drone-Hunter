...
def get_model_sparsity(model_file, mode, dataset,
    ...
    try:
        ...
        print("EI_LOG_LEVEL=error ERROR: Can't calculate model sparsity: " + str(err), flush=True)
        return 0
...
def get_macs_flops(model: tf.keras.Model, verbose=False):
    ...
    try:
        ...
        print(f"EI_LOG_LEVEL=warn WARNING: Can't calculate FLOPS: {err}", flush=True)
        flops = 0
...
def get_hardware_utilization(model_file):
    ...
    try:
        ...
        print("EI_LOG_LEVEL=error ERROR: Can't map model to AKD1000 NSoC! Can't calculate program size, number of NPs and nodes!", flush=True)
        print("EI_LOG_LEVEL=error ERROR: " + str(err), flush=True)
    ...
    try:
        ...
        print("EI_LOG_LEVEL=warn WARNING: Requested model can't be fully mapped to hardware. Reason:", flush=True)
        print("EI_LOG_LEVEL=warn WARNING: " + str(err), flush=True)
        print("EI_LOG_LEVEL=warn WARNING: Reported program size, number of NPs and nodes may not be accurate!", flush=True)
...
def process_output_yolov2(output_data, img_shape, num_classes, anchors, minimum_confidence_rating=None):
    ...
    nms_scores = list(zip(pred_boxes, pred_labels, score))

    # sort nms_scores by score in descending order
    nms_scores.sort(key=lambda x: x[2], reverse=True)
